...
namespace SDE.Databases.Generic.Controls {
	public partial class ClientQuestImage : UserControl {
		public Image Image => _image;
		private readonly GrfImageWrapper _wrapper = new GrfImageWrapper();

		public ClientQuestImage() {
			InitializeComponent();
		...
		public void SetIcon(string path) {
			try {
				const string IconPath = @"data\texture\À¯ÀúÀÎÅÍÆäÀÌ½º\renew_questui";
				var current = (SourceField ?? "").ExpandString();
				byte[] data = SdeEditor.MetaGrf.GetDataBuffered(EncodingService.FromAnyToDisplayEncoding(GrfPath.Combine(IconPath, current)));
