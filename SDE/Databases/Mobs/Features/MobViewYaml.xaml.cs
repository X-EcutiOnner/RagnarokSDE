...
namespace SDE.Databases.Mobs.Features {
	...
	/// </summary>
	public partial class MobViewYaml : UserControl, IDatabaseView {
		...
		private bool _editDrop(ItemDropViewModel vm, ItemDrop model, bool isMvp, bool isNew = false) {
			try {
				DropEditDialog dialog = new DropEditDialog(model.Item ?? "", model.Rate ?? "0", DataSources.Item, !isMvp, isMvp ? DropEditFlag.RandGroup : DropEditFlag.RandGroup | DropEditFlag.StealProtected);
				...
				if (dialog.ShowDialog() == true) {
					...
					try {
						...
						if (isNew) {
							...
							_tab.Table.Commands.SetModelValue(_viewModel.Tuple, model, nameof(ItemDrop.RandomOptionGroup), dialog.RandGroup);
							_tab.Table.Commands.SetModelValue(_viewModel.Tuple, model, nameof(ItemDrop.StealProtected), dialog.StealProtected);
		...
		private ItemDrop _addDrop(bool isMvp) {
			ItemDrop itemDrop = new ItemDrop();
			ItemDropViewModel vm = new ItemDropViewModel(_viewModel, itemDrop);
