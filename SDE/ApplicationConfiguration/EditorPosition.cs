...
namespace SDE.ApplicationConfiguration {
	public class EditorPosition {
		...
		public void Load(SdeEditor editor) {
			FromString(SdeAppConfiguration.EditorSavedPositions);

			if (!IsSet || !SdeAppConfiguration.SaveEditorPosition)
				return;
		...
		public void Save(SdeEditor editor) {
		...
		public void Save() {
			SdeAppConfiguration.EditorSavedPositions = ToString();
		}
	}
}
