...
namespace SDE.View.Editors {
	...
	public partial class LevelEnumDialog : TkWindow, IInputWindow {
		...
		private List<EnumInfoBase> _values = new List<EnumInfoBase>();
		...
		public LevelEnumDialog(string text, object maxLevel, bool autoFill, Type enumType) : base("Level edit", "cde.ico", SizeToContent.Height, ResizeMode.CanResize) {
			...
			int max;
			_values = EnumInfos.GetEnumInfoList(enumType);
			...
			var prefix = EnumInfos.GetMarker(enumType);
			var enumTable = EnumInfos.GetEnumTypeToInfo(enumType);
			...
			for (int i = 0; i < max; i++) {
				...
				if (i < values.Length) {
					try {
						if (EnumInfos.TryStringToEnum(enumType, text, prefix, out Enum r)) {
							box.SelectedItem = enumTable[r];
						}
					}
					catch {
						box.SelectedIndex = -1;
		...
		private string _box2String(ComboBox box) {
			...
			var value = _values[box.SelectedIndex];

			return value.YamlName;
		}
