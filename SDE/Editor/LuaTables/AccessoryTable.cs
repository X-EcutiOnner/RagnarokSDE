...
namespace SDE.Editor.LuaTables {
	...
	public class AccessoryTable {
		...
		private readonly MergedTable _itemDb;
		private readonly Table<int, ReadableTuple> _cItemDb;

		private readonly List<ReadableTuple> _headgears;
		...
		public AccessoryTable(BaseDatabase db, byte[] dataAccId, byte[] dataAccName) {
			...
			_cItemDb = SdeEditor.Project.GetTable(DataSources.ClientItem);
			...
			_headgears = _itemDb.FastItems.Where(p => p.GetModel<Item>().Type == Databases.Items.Common.ItemType.IT_ARMOR && (p.GetModel<Item>().Locations.ToLong() & 7937) != 0).OrderBy(p => p.GetModel<Item>().View.ToInt()).ToList();
		...
		public void SetDbs() {
			foreach (var tuple in _headgears) {
				...
				var serverModel = tuple.GetModel<Item>();
				var sViewId = serverModel.View.ToInt();
				...
				if (cTuple != null) {
					var clientModel = cTuple.GetModel<ClientItem>();
					var cViewId = DbReader.ToInt(clientModel.ClassNumber);
					var sprite = cTuple.GetModel<ClientItem>().IdentifiedResourceName ?? "".ToDisplayEncoding();
					...
					if (sViewId != nViewId) _itemDb.Commands.SetModelValue(tuple, serverModel, nameof(Item.View), nViewId.ToString(CultureInfo.InvariantCulture));
					if (cViewId != nViewId) _cItemDb.Commands.SetModelValue(cTuple, clientModel, nameof(ClientItem.ClassNumber), nViewId.ToString(CultureInfo.InvariantCulture));
				}
			}
		}
		...
		private void _setLuaFromHeadgears() {
			foreach (var tuple in _headgears) {
				...
				if (cTuple != null) {
					var sprite = cTuple.GetModel<ClientItem>().IdentifiedResourceName ?? "";
					var entry = _getEntryFromSprite(sprite);
					...
					var tViewId = tuple.GetModel<Item>().View.ToInt();
		...
		private void _setLuaFromOverwrite() {
			foreach (var keyPair in ItemIdToSprite) {
				...
				var clientModel = cTuple?.GetModel<ClientItem>();
				var serverModel = sTuple?.GetModel<Item>();
				...
				if (sprite != null) {
					...
					if (serverModel != null && serverModel.View.ToInt() != viewId) _itemDb.Commands.SetModelValue(sTuple, serverModel, nameof(Item.View), viewId.ToString(CultureInfo.InvariantCulture));
					if (clientModel != null && clientModel.ClassNumber.ToInt() != viewId) _cItemDb.Commands.SetModelValue(cTuple, clientModel, nameof(ClientItem.ClassNumber), viewId.ToString(CultureInfo.InvariantCulture));
				}
				else {
					// Invisible items
					if (serverModel != null && serverModel.View.ToInt() != 0) _itemDb.Commands.SetModelValue(sTuple, serverModel, nameof(Item.View), "0");
					if (clientModel != null && clientModel.ClassNumber.ToInt() != 0) _cItemDb.Commands.SetModelValue(cTuple, clientModel, nameof(ClientItem.ClassNumber), "0");
				}
			}
		}
