...
namespace SDE.Editor.Parsers {
	public static class TextFileHelper {
		...
		public static string LastLineRead2;
			...
			if (data != null) {
				using (StreamReader reader = SetAndLoadReader(data, EncodingService.DetectEncoding(data))) {
					...
					while (!reader.EndOfStream) {
						...
						if (elements != null) {
							if (int.TryParse(subElements[0], out ival)) {
					...
					if (elements != null && int.TryParse(elements[0], out ival)) {
						yield return elements;
					}
				}
			}
		...
		public static IEnumerable<string[]> GetElementsByCommasAll(byte[] data) {
			if (data != null) {
				using (StreamReader reader = SetAndLoadReader(data)) {
					while (!reader.EndOfStream) {
						string line = ReadNextLine(reader);
						...
						if (!string.IsNullOrEmpty(line)) {
							LastLineRead = line;
							yield return ExcludeBracketsAll(line.Trim('\t'));
						}
					}
				}
			}
		...
		public static string[] ExcludeBracketsAll(string line) {
