...
namespace SDE.Editor.Parsers.Libconfig {
	...
	public class LibconfigParser {
		...
		public void Write2(string key, string line) {
			if (!_list) {
				...
				if (conf == null) {
					// Add a new one!
					_writeKeyValues.Add(new ParserKeyValue(key, int.MaxValue) {
				...
				if (conf == null) {
					_writeArrays.Add(new ParserArray(int.MaxValue) {
		...
		public void Remove<TKey>(BaseDatabase db) {
			if (db.Table.Commands.GetUndoCommands() == null)
				return;

			foreach (GroupCommand<TKey, ReadableTuple> command in db.Table.Commands.GetUndoCommands().OfType<GroupCommand<TKey, ReadableTuple>>()) {
				foreach (DeleteTuple<TKey, ReadableTuple> deleteCommand in command.Commands.OfType<DeleteTuple<TKey, ReadableTuple>>()) {
					Delete(deleteCommand.Key);
				}

				foreach (ChangeTupleKey<TKey, ReadableTuple> changeTupleKeyCommand in command.Commands.OfType<ChangeTupleKey<TKey, ReadableTuple>>()) {
					// If the key was changed, the old key must be removed
					Delete(changeTupleKeyCommand.Key);
				}
			}

			foreach (ChangeTupleKey<TKey, ReadableTuple> command in db.Table.Commands.GetUndoCommands().OfType<ChangeTupleKey<TKey, ReadableTuple>>()) {
				// If the key was changed, the old key must be removed
				Delete(command.Key);
			}
		}
		...
		public void WriteFile(string path) {
			...
			if (!_list) {
				...
				_writeArrays = _writeArrays.OrderBy(p => int.Parse(p[_idKey])).ToList();
