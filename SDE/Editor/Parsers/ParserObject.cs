...
namespace SDE.Editor.Parsers {
	public class ParserObject : IEnumerable<ParserObject> {
		...
		public ParserObject this[string key] {
			get {
				...
				if (this is ParserArrayBase arrayBase) {
					if (arrayBase.InternalKeyValues != null) {
						if (arrayBase.InternalKeyValues.TryGetValue(key, out ParserObject res))
							return res;
					}
					else {
						if ((keyValue = arrayBase.Objects.OfType<ParserKeyValue>().FirstOrDefault(p => p.Key == key)) != null)
							return keyValue.Value;
					}
				}
		...
		public void IndexProperties() {
			if (this is ParserArrayBase arrayBase) {
				arrayBase.InternalKeyValues = new Dictionary<string, ParserObject>();
				var dico = arrayBase.InternalKeyValues;

				foreach (var keyValue in arrayBase.Objects.OfType<ParserKeyValue>()) {
	...
	public class ParserArrayBase : ParserObject {
		...
		internal Dictionary<string, ParserObject> InternalKeyValues;
